...
export const Schedule: React.FC = () => {
  ...
  return (
    <section id="schedule" className="relative py-24 sm:py-32 overflow-hidden bg-[#050508] border-t border-slate-900">
      {/* Background Accent Grid */}
      <div className="absolute inset-0 bg-cyber-dots opacity-20 pointer-events-none" />
      <div className="absolute top-1/4 right-0 w-96 h-96 bg-red-600/10 rounded-full blur-3xl pointer-events-none -z-10" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Section Heading */}
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-14">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-900 border border-red-500/30 text-red-300 text-xs sm:text-sm font-semibold tracking-wider font-rajdhani uppercase">
          ...
          <h2 className="font-orbitron text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight">
            Five Days. <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-rose-400 to-amber-400">One Campus.</span>
          </h2>
            ...
            return (
              <button
                ...
                className={`relative px-4 sm:px-6 py-3 rounded-2xl font-orbitron text-xs sm:text-sm font-bold tracking-wider transition-all duration-200 flex flex-col items-center gap-1 focus:outline-none ${
                  isActive
                    ? 'bg-gradient-to-b from-red-600/30 via-slate-900 to-slate-950 border border-red-500 text-white shadow-[0_0_25px_rgba(255,23,68,0.3)] scale-105'
                    : 'bg-slate-900/80 border border-slate-800 text-slate-400 hover:text-slate-200 hover:border-slate-700'
                }`}
              >
                <span className={`text-[10px] uppercase font-rajdhani font-bold ${isActive ? 'text-amber-400' : 'text-slate-500'}`}>
                  DAY 0{day.dayNumber}
                </span>
                <span className="text-white font-extrabold text-sm sm:text-base">
                  {day.dateStr}
                </span>
                <span className={`text-[10px] font-rajdhani max-w-[120px] truncate ${isActive ? 'text-red-300 font-semibold' : 'text-slate-500'}`}>
                  {day.theme}
                </span>
              </button>
            );
        ...
        <div className="mb-8 p-4 sm:p-6 rounded-2xl bg-slate-900/90 border border-red-500/30 flex flex-col md:flex-row items-start md:items-center justify-between gap-4 backdrop-blur-xl">
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              <span className="text-xs font-mono px-2.5 py-0.5 rounded bg-red-500/20 text-red-300 border border-red-500/40">
            ...
            <p className="text-xs sm:text-sm text-amber-400 font-rajdhani font-semibold uppercase tracking-wider">
              {currentDay.theme}
            </p>
          </div>
          ...
          <div className="flex items-center gap-2 self-stretch md:self-auto">
            ...
            <select
              ...
              className="w-full md:w-auto px-3 py-2 rounded-xl bg-slate-950 border border-slate-800 text-xs text-slate-200 font-rajdhani font-semibold focus:outline-none focus:border-red-500 transition-colors"
            >
              {VENUES.map((v) => (
        ...
        <div className="relative border-l-2 border-slate-800/80 ml-4 sm:ml-8 space-y-6 sm:space-y-8 pl-6 sm:pl-8">
          {filteredEvents.length > 0 ? (
            filteredEvents.map((item, idx) => (
              <div
                key={idx}
                className="relative group p-5 sm:p-6 rounded-2xl bg-slate-900/70 border border-slate-800 hover:border-red-500/50 transition-all duration-300 hover:shadow-[0_4px_25px_rgba(0,0,0,0.5)] backdrop-blur-md"
              >
                {/* Timeline node marker */}
                <div className="absolute -left-[33px] sm:-left-[41px] top-6 w-4 h-4 rounded-full bg-slate-900 border-2 border-red-500 group-hover:bg-red-500 group-hover:shadow-[0_0_12px_#ff1744] transition-all" />

                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                  {/* Event Details */}
                  <div className="space-y-2 flex-1">
                    <div className="flex flex-wrap items-center gap-2">
                      <span className="px-2.5 py-0.5 rounded text-[10px] font-mono font-bold bg-red-500/15 text-red-300 border border-red-500/30">
                        {item.category}
                      </span>
                      {item.tag && (
                        <span className="px-2 py-0.5 rounded text-[10px] font-orbitron font-semibold bg-rose-500/20 text-rose-300 border border-rose-500/40">
                          {item.tag}
                        </span>
                      )}
                    </div>

                    <h4 className="font-orbitron font-bold text-lg sm:text-xl text-white group-hover:text-red-400 transition-colors">
                      {item.title}
                    </h4>
                  ...
                  <div className="shrink-0 flex items-center">
                    <a
                      ...
                      className="px-3.5 py-2 rounded-xl bg-slate-950 border border-slate-800 hover:border-red-500/80 hover:bg-slate-900 text-slate-300 hover:text-red-300 text-xs font-rajdhani font-bold tracking-wider uppercase flex items-center gap-2 transition-all shadow-sm group-hover:border-red-500/40"
                    >
            ...
            <div className="p-8 rounded-2xl bg-slate-900/40 border border-slate-800 text-center space-y-2">
              ...
              <button
                ...
                className="text-xs font-orbitron text-red-400 hover:underline"
              >
                Reset to All Venues
              </button>
            </div>
