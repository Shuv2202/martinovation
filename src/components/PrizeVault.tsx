...
export const PrizeVault: React.FC<PrizeVaultProps> = ({ onOpenRegister }) => {
  ...
  return (
    <section id="prizes" className="relative py-24 sm:py-32 overflow-hidden bg-[#050508] border-t border-slate-900">
      {/* Ambient background glows */}
      <div className="absolute top-1/2 left-1/3 w-96 h-96 bg-red-600/10 rounded-full blur-[140px] pointer-events-none -z-10" />
      <div className="absolute bottom-0 right-10 w-80 h-80 bg-rose-600/10 rounded-full blur-[130px] pointer-events-none -z-10" />

      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        {/* Heading */}
        <div className="text-center max-w-3xl mx-auto space-y-4 mb-16">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-900 border border-red-500/40 text-red-400 text-xs sm:text-sm font-semibold tracking-wider font-rajdhani uppercase">
          ...
          <h2 className="font-orbitron text-3xl sm:text-4xl lg:text-5xl font-black text-white tracking-tight">
            Grand <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-rose-400 to-amber-400">Prize Vault</span>
          </h2>
            ...
            return (
              <div
                ...
                className={`p-6 rounded-2xl border cursor-pointer transition-all duration-300 flex flex-col justify-between ${
                  isCurrent
                    ? 'bg-slate-900 border-red-500/80 shadow-[0_0_25px_rgba(255,23,68,0.25)] -translate-y-1'
                    : 'bg-slate-950/60 border-slate-800/80 hover:bg-slate-900/60 hover:border-slate-700'
                }`}
              >
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <div className={`p-3 rounded-xl ${isCurrent ? 'bg-red-600 text-white shadow-lg font-black' : 'bg-slate-900 text-red-400 border border-slate-800'}`}>
                  ...
                  <div>
                    ...
                    <h3 className="font-orbitron font-bold text-sm text-amber-400 mt-1">
                      {cat.title}
                    </h3>
                  </div>
                ...
                <div className="pt-4 mt-4 border-t border-slate-800/80 text-[11px] text-slate-400 font-rajdhani flex items-center justify-between">
                  ...
                  <span className={`w-2 h-2 rounded-full ${isCurrent ? 'bg-red-500 animate-ping' : 'bg-slate-700'}`} />
                </div>
              </div>
            );
          })}
        ...
        <div className="mt-8 p-6 sm:p-8 rounded-3xl bg-slate-900/90 border border-red-500/30 backdrop-blur-xl shadow-2xl flex flex-col lg:flex-row items-center justify-between gap-6">
