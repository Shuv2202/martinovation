      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 max-w-5xl mx-auto">
        ...
        <div className="relative w-full sm:w-80">
          ...
          <input
            ...
            className="w-full pl-10 pr-10 py-2 rounded-xl bg-slate-900/80 border border-slate-800 text-white placeholder-slate-400 text-xs focus:outline-none focus:ring-1 focus:ring-cyan-400 focus:border-cyan-400 transition-all backdrop-blur-md"
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
          <div className="text-xs font-rajdhani font-semibold text-slate-400 flex items-center gap-2">
            <span>Arenas:</span>
            <span className="px-2 py-0.5 rounded bg-cyan-500/10 border border-cyan-500/30 text-cyan-300 font-mono">
              {totalResults}
            </span>
          </div>
          ...
          return (
            <button
              ...
              className={`px-3.5 py-1.5 rounded-full font-orbitron text-[11px] font-semibold tracking-wider transition-all duration-200 flex items-center gap-1.5 ${
                isActive
                  ? 'bg-gradient-to-r from-cyan-400 to-cyan-500 text-black shadow-[0_0_15px_rgba(0,240,255,0.35)] scale-105'
                  : 'bg-slate-900/70 border border-slate-800 text-slate-300 hover:text-white hover:border-slate-700'
              }`}
            >
              <span>{category}</span>
              <span className={`text-[9px] px-1.5 py-0.2 rounded-full font-mono ${
                isActive ? 'bg-black/30 text-black font-black' : 'bg-slate-800 text-slate-400'
              }`}>
                {count}
              </span>
            </button>
